...
\begin{definition}[Right continuous function]\label{def:RightContinuous}
  \leanok
  \lean{Function.RightContinuous}
  Let $T$ be equipped with a partial order and $f : T \to E$ be a function. Then $f$ is right
  continuous if for all $t \in T$, $\lim_{s \downarrow t} f(s) = f(t)$.
\end{definition}

\begin{definition}[Cadlag function]\label{def:IsCadlag}
  \uses{def:RightContinuous}
  \leanok
  \lean{IsCadlag}
  A function $f : T \to E$ is cadlag if it is right continuous and the limit $\lim_{s \uparrow t} f(s)$ exists for all $t \in T$.
\end{definition}


\begin{lemma}\label{lem:Adapted.progMeasurable_of_rightContinuous}
  \uses{def:adapted, def:ProgMeasurable, def:RightContinuous}
  \leanok
  \lean{MeasureTheory.StronglyAdapted.progMeasurable_of_rightContinuous}
...
\begin{proof}
  \leanok
  Fixing $t \in T$, let $S$ be a union of a dense set of $(-\infty,t]$ and the set of points isolated from right in $(-\infty,t]$.
  As $T$ is second countable, $S$ can be chosen so that it is countable, which means that there exists a surjective function $u: \mathbb{N} \rightarrow S$.
  We need to show that $X$ restricted to $(-\infty, t] \times \Omega$ is measurable with respect to $\mathcal{B}((-\infty, t]) \otimes \mathcal{F}_t$.
  To this end, we rearrange the image of the first $n$ numbers under the map $u$ in nondecreasing order, denoted by $v_0\le v_1\le \cdots\le v_n$,
  ...
  Thus, as each $X^n$ is measurable, it follows that $X$ is also measurable (by using \verb|stronglyMeasurable_of_tendsto|).
\end{proof}
...
\begin{lemma}[Jensen's inequality for the conditional expectation]\label{lem:conditional_jensen}
  \mathlibok
  \lean{ConvexOn.map_condExp_le_univ}
...
\begin{lemma}\label{lem:rightContinuousRightCont}
  \uses{def:rightLimitFiltration, def:rightContinuous}
  \mathlibok
  \lean{MeasureTheory.Filtration.instIsRightContinuousRightCont}
...
\begin{lemma}\label{lem:Adapted.isPredictable_of_leftContinuous}
  \uses{def:predictable}
An adapted process with left-continuous paths is predictable.
\end{lemma}
...
\begin{lemma}\label{lem:uniformIntegrableDominatedSingleton}
  \leanok
  \lean{MeasureTheory.uniformIntegrable_of_dominated_singleton} % and MeasureTheory.uniformIntegrable_of_dominated_enorm_singleton
Let $(X_t)_{t \in T}$ be a family of random variables and $Y$ be a real random variable in $L^p$. If for all $t$, (possibly infinite) $\|X_t\| \le Y$ almost surely, then $X$ is uniformly integrable.
\end{lemma}
...
\begin{proof}
  \uses{def:martingalePart, lem:martingale_martingalePart, def:predictablePart, lem:predictable_predictablePart, lem:uniformIntegrable_stoppedValue_martingale, lem:uniformIntegrableAdd, lem:uniformIntegrableDominatedSingleton, lem:predictablePart_zero, lem:Submartingale.monotone_predictablePart, lem:centering_basic}
...
\begin{lemma}\label{lem:uniformIntegrable_of_tendsto_ae}
  \leanok
  \lean{MeasureTheory.UniformIntegrable.uniformIntegrable_of_ae_tendsto}
...
\begin{definition}[Approximable time index]\label{def:approximableTimeIndex}
  \uses{def:IsStoppingTime, def:approxSeq}
  \leanok
  \lean{MeasureTheory.Approximable}
  A time index set \(T\) is said to be approximable if for any stopping time \(\tau : \Omega \to T \cup \{\infty\}\), there exists a discrete approximation sequence \((\tau_n)\) of \(\tau\).
\end{definition}

\begin{lemma}\label{lem:tendsto_stoppedValue_discreteApproxSequence}
  \uses{def:IsStoppingTime, def:approxSeq, def:RightContinuous}
  \leanok
...
\begin{lemma}\label{lem:tendsto_eLpNorm_stoppedValue_discreteApproxSequence}
  \uses{def:RightContinuous, def:IsStoppingTime, def:approxSeq}
  \leanok
...
\begin{lemma}\label{lem:stoppingTime_approximation}
  \uses{def:IsStoppingTime, def:approximableTimeIndex}
  \leanok
  \lean{NNReal.approximable}
  $T = \mathbb{R}_+$ is an approximable time index. In particular,
  for any stopping time $\tau$ on $\overline{\mathbb{R}_+}$, defining $\tau_n =  2^{-n} \lceil 2^n \tau \rceil$,
...
\begin{lemma}\label{lem:stoppingTime_approximationNat}
  \uses{def:IsStoppingTime, def:approximableTimeIndex}
  \leanok
  \lean{Nat.approximable}
  $T = \mathbb{N}$ is an approximable time index.
\end{lemma}
\begin{proof}
  Immediate as we can take \(\tau_n = \tau\) for all \(n\).
\end{proof}

\begin{lemma}[Optional sampling (continuous time)]\label{lem:optionalSampling}
  \uses{def:Martingale, def:IsStoppingTime, def:RightContinuous, def:approximableTimeIndex}
  \leanok
  \lean{MeasureTheory.Martingale.stoppedValue_min_ae_eq_condExp'}
  Let $X$ be a right-continuous $\mathcal{F}$-martingale on an approximable time index. Then, for
...
\begin{proof}
  \leanok
  \uses{lem:StoppingTimeGenMono, lem:uniformIntegrable_stoppedValue_martingale,
  ...
  Let $(\tau_n), (\sigma_n)$ be discrete approximation sequences of $\tau$ and $\sigma$ respectively.
...
\begin{lemma}\label{lem:optionalSamplingSubmartingale}
  \uses{def:Submartingale, def:IsStoppingTime, def:RightContinuous, def:approximableTimeIndex}
  \leanok
  \lean{MeasureTheory.Submartingale.stoppedValue_min_ae_le_condExp}
  Let $X$ be a right-continuous $\mathcal{F}$-submartingale on an approximable time index. Then, for
  any stopping times $\sigma, \tau$ with $\tau$ bounded, we have that
...
\begin{theorem}\label{thm:tendsto_limitProcess_of_cadlag}
  \uses{def:Martingale, def:limitProcess, def:IsCadlag}
  \notready
...
\begin{lemma}[Doob's maximal Inequality for countable]\label{lem:doob_countable}
  \uses{def:Submartingale}
  \leanok
  \lean{MeasureTheory.Submartingale.ae_bddAbove_Iic, ProbabilityTheory.maximal_ineq_countable}
...
\begin{align*}
  ...
  \: .
\end{align*}
Then we build a countable increasing sequence of finite sets $J_n$ with $\bigcup_nJ_n=I$.
Note for any $\lambda' < \lambda$,
\begin{align*}
  \lambda' P\left( \sup_{i\in I, i\leq M}X_i>\lambda' \right)
  &= \lambda' P\left( \bigcup_{i\in I, i\leq M}\{X_i>\lambda'\} \right) \\
  &= \lambda' \lim_{n\to\infty} P\left( \bigcup_{i\in J_n, i\leq M}\{X_i>\lambda'\} \right) \\
  &\le \lim_{n\to\infty} \mathbb{E}\left[X_{M} \mathbb{I}_{\sup_{i \in J_n, i \le M}X_i \ge \lambda'}\right] \\
  &\le \mathbb{E}\left[X_{M} \mathbb{I}_{\sup_{i \in I, i \le M}X_i \ge \lambda'}\right]
\end{align*}
Now take $\lambda'\uparrow\lambda$ and use monotone or dominated convergence to derive the desired inequality.

In the above, we first interpret $\sup_{i\in I, i\leq M}X_i$ in extended reals for well-definedness,
then observe by the (now proved) maximal inequality that $\sup_{i\in I, i\leq M}X_i < \infty$ a.s.\ by taking $\lambda\to\infty$,
so the supremum is well-defined in $\mathbb{R}$.
%See 8.1.1 Pascucci.
\end{proof}
...
\begin{theorem}[Doob Inequality]\label{thm:doob_ineq}
  \uses{def:Submartingale, def:RightContinuous}
  \leanok
...
\begin{corollary}[Doob Inequality for normed spaces]\label{cor:doob_ineq_norm}
  \uses{def:Martingale, def:RightContinuous}
  \leanok
...
\begin{lemma}[Stopped Submartingale]\label{lem:Submartingale.stoppedProcess_of_cadlag}
  \uses{def:Submartingale, def:stoppedProcess, def:IsCadlag, def:IsStoppingTime}
