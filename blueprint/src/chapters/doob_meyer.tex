...
\[ \tilde{g}_n^{k+1} = \sum_{l} (\lambda^{k,n}_\cdot * \ldots * \lambda^{1,\cdot}_\cdot)_l x^{(k+1)}_l \]
We apply Lemma~\ref{lem:komlos_norm} to $(\tilde{g}_n^{k+1})_{n\in\mathbb{N}}$ to get $g_n^{k+1} \in convex(\tilde{g}_n^{k+1}, \tilde{g}_{n+1}^{k+1}, \ldots)$ such that $g_n^{k+1}$ converges to some $g^{k+1}$. We denote its weights by $\lambda^{k+1,n}_n,\cdots,\lambda^{k+1,n}_{N^{k+1}_n}$ and can then write:

\begin{align*}
  g_n^{k+1} & = \sum_{m} \lambda^{k+1,n}_m \tilde{g}_m^{k+1}
