...
\begin{lemma}\label{lem:todo}
  \uses{def:predictablePart, def:classD}
Let $T \in \mathbb{N}$ and let $X$ be an adapted process with $X_n$ integrable for all $n$.
Then for a stopping time $\tau \le T$, almost surely, $M_\tau = \mathbb{E}[X_T - A_T \mid \mathcal{F}_\tau]$ and $X_\tau = A_\tau + \mathbb{E}[X_T - A_T \mid \mathcal{F}_\tau]$.
\end{lemma}

\begin{proof}
  \uses{def:martingalePart, lem:martingale_martingalePart}
By definition $M_T = X_T - A_T$.
Since $M$ is a martingale it follows by optional sampling that for any stopping time $\tau \le T$
\begin{align*}
  M_\tau = \mathbb{E}[M_T \mid \mathcal{F}_\tau] = \mathbb{E}[X_T - A_T \mid \mathcal{F}_\tau]
  \: .
\end{align*}
And then $X_\tau = M_\tau + A_\tau = A_\tau + \mathbb{E}[X_T - A_T \mid \mathcal{F}_\tau]$.
\end{proof}


\begin{lemma}\label{lem:uniformIntegrable_predictablePart}
  \uses{def:predictablePart, def:classD}
  Let $X$ be a submartingale of class D with $X \le 0$ a.s. and $X_\infty = 0$ and let $A$ be its predictable part.
  The sequence $(A^n_T)_{n\in\mathbb{N}}$ is uniformly integrable (bounded in $L^1$ norm).
\end{lemma}

\begin{proof}
  \uses{lem:predictable_predictablePart,lem:Submartingale.monotone_predictablePart,lem:martingale_martingalePart,lem:IsStoppingTime_leastGT_predictablePart,lem:leastGT_predictablePart_sub_ge}
...
\begin{proof}
  \uses{def:Submartingale, lem:Submartingale.monotone_predictablePart}
